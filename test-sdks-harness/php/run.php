...
{
    foreach ([
        'Svix\\ApiInternal\\MessagePollerv2ConsumerPollOptions',
        'Svix\\ApiInternal\\MessagePollerv2\\MessagePollerv2ConsumerPollOptions',
    ] as $class) {
        if (class_exists($class)) {
            $opts = new $class();
            if (property_exists($opts, 'startingPosition')) {
                $opts->startingPosition = 'earliest';
            }
            if (property_exists($opts, 'leaseDurationMs')) {
                $opts->leaseDurationMs = 2000;
            }
    ...
    throw new RuntimeException('MessagePollerv2ConsumerPollOptions class not found');
}
